using System;
using System.Collections;
using System.Collections.Generic;
using ModestTree.Util;
using NUnit.Framework;
using UnityEngine;
using UnityEngine.TestTools;
using Zenject.Tests.TestAnimationStateBehaviourInject;

namespace Zenject.Tests.Misc.TestMonoKernelDecoration
{
    public sealed class TestMonoKernelDecoration : ZenjectIntegrationTestFixture
    {
        [UnityTest]
        public IEnumerator TestDelayedMonoKernelDecorator()
        {
            PreInstall();

            Container.Rebind<InitializableManager>()
                     .To<InitializableManagerSpy>()
                     .AsCached();
            
            KernelDecoratorInstaller.Install(Container);
            PostInstall();
            
            yield return new WaitForSeconds(1.0f);

            var initializableManager = SceneContext.Container.Resolve<InitializableManager>() as InitializableManagerSpy;
            bool initializedBeforeDelay = initializableManager.IsInitialized;
            
            yield return new WaitForSeconds(6.0f);
            bool initializedAfterDelay = initializableManager.IsInitialized;

            Assert.IsFalse(initializedBeforeDelay);
            Assert.IsTrue(initializedAfterDelay);
        }
        
        private sealed class InitializableManagerSpy : InitializableManager
        {
            public InitializableManagerSpy(List<IInitializable> initializables, 
                                           List<ValuePair<Type, int>> priorities,
                                           List<ILateInitializable> lateInitializables, 
                                           List<ValuePair<Type, int>> latePriorities)
                : base(initializables, priorities, lateInitializables, latePriorities)
            {
            }

            public bool IsInitialized => hasInitialized;
        }
    }
}